...
catch(ex){if(msg.errback){pm.send({target:source_window,data:ex,type:msg.errback,hash:true,url:hash.source.url});}else{throw ex;}}};}},_url:function(url){return(""+url).replace(/#.*$/,"");}};$.extend(pm,{defaults:{target:null,url:null,type:null,data:null,success:null,error:null,origin:"*",hash:false}});})(this,typeof jQuery==="undefined"?NO_JQUERY:jQuery);if(!("JSON"in window&&window.JSON)){JSON={}}(function(){function f(n){return n<10?"0"+n:n}if(typeof Date.prototype.toJSON!=="function"){Date.prototype.toJSON=function(key){return this.getUTCFullYear()+"-"+f(this.getUTCMonth()+1)+"-"+f(this.getUTCDate())+"T"+f(this.getUTCHours())+":"+f(this.getUTCMinutes())+":"+f(this.getUTCSeconds())+"Z"};String.prototype.toJSON=Number.prototype.toJSON=Boolean.prototype.toJSON=function(key){return this.valueOf()}}var cx=/[\u0000\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g,escapable=/[\\\"\x00-\x1f\x7f-\x9f\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g,gap,indent,meta={"\b":"\\b","\t":"\\t","\n":"\\n","\f":"\\f","\r":"\\r",'"':'\\"',"\\":"\\\\"},rep;function quote(string){escapable.lastIndex=0;return escapable.test(string)?'"'+string.replace(escapable,function(a){var c=meta[a];return typeof c==="string"?c:"\\u"+("0000"+a.charCodeAt(0).toString(16)).slice(-4)})+'"':'"'+string+'"'}function str(key,holder){var i,k,v,length,mind=gap,partial,value=holder[key];if(value&&typeof value==="object"&&typeof value.toJSON==="function"){value=value.toJSON(key)}if(typeof rep==="function"){value=rep.call(holder,key,value)}switch(typeof value){case"string":return quote(value);case"number":return isFinite(value)?String(value):"null";case"boolean":case"null":return String(value);case"object":if(!value){return"null"}gap+=indent;partial=[];if(Object.prototype.toString.apply(value)==="[object Array]"){length=value.length;for(i=0;i<length;i+=1){partial[i]=str(i,value)||"null"}v=partial.length===0?"[]":gap?"[\n"+gap+partial.join(",\n"+gap)+"\n"+mind+"]":"["+partial.join(",")+"]";gap=mind;return v}if(rep&&typeof rep==="object"){length=rep.length;for(i=0;i<length;i+=1){k=rep[i];if(typeof k==="string"){v=str(k,value);if(v){partial.push(quote(k)+(gap?": ":":")+v)}}}}else{for(k in value){if(Object.hasOwnProperty.call(value,k)){v=str(k,value);if(v){partial.push(quote(k)+(gap?": ":":")+v)}}}}v=partial.length===0?"{}":gap?"{\n"+gap+partial.join(",\n"+gap)+"\n"+mind+"}":"{"+partial.join(",")+"}";gap=mind;return v}}if(typeof JSON.stringify!=="function"){JSON.stringify=function(value,replacer,space){var i;gap="";indent="";if(typeof space==="number"){for(i=0;i<space;i+=1){indent+=" "}}else{if(typeof space==="string"){indent=space}}rep=replacer;if(replacer&&typeof replacer!=="function"&&(typeof replacer!=="object"||typeof replacer.length!=="number")){throw new Error("JSON.stringify")}return str("",{"":value})}}if(typeof JSON.parse!=="function"){JSON.parse=function(text,reviver){var j;function walk(holder,key){var k,v,value=holder[key];if(value&&typeof value==="object"){for(k in value){if(Object.hasOwnProperty.call(value,k)){v=walk(value,k);if(v!==undefined){value[k]=v}else{delete value[k]}}}}return reviver.call(holder,key,value)}cx.lastIndex=0;if(cx.test(text)){text=text.replace(cx,function(a){return"\\u"+("0000"+a.charCodeAt(0).toString(16)).slice(-4)})}if(/^[\],:{}\s]*$/.test(text.replace(/\\(?:["\\\/bfnrt]|u[0-9a-fA-F]{4})/g,"@").replace(/"[^"\\\n\r]*"|true|false|null|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?/g,"]").replace(/(?:^|:|,)(?:\s*\[)+/g,""))){j=eval("("+text+")");return typeof reviver==="function"?walk({"":j},""):j}throw new SyntaxError("JSON.parse")}}}());function on(el,eventName,handler){if(el.addEventListener){el.addEventListener(eventName,handler);}else{el.attachEvent('on'+eventName,function(){handler.call(el);});}}
var googlePlusButtonHasBeenClicked=false;on(document.getElementById('customButton'),'click',googlePlusOnClickCustomButton);function googlePlusOnClickCustomButton(){googlePlusButtonHasBeenClicked=true;pm({target:window.parent,type:'googlePlusButtonClick',data:0,origin:originUrl});}
function googlePlusSignInCallback(authResult){if(googlePlusButtonHasBeenClicked){if('undefined'!=typeof authResult.error){googlePlusSignInEvent({error:authResult.error});}else{googlePlusGetProfile(function(profileArgs){var data={};data.code=authResult.code;data.access_token=authResult.access_token;data.name=profileArgs.displayName;data.avatar=profileArgs.image.url;data.link=profileArgs.url;data.uid=profileArgs.id;data.email=profileArgs['emails'].filter(function(v){return v.type==='account';})[0].value;googlePlusSignInEvent(data);});}}}
function googlePlusGetProfile(callback){gapi.client.load('plus','v1',function(){var request=gapi.client.plus.people.get({userId:'me'});request.execute(function(resp){callback(resp);});});}
function googlePlusSignInEvent(data){pm({target:window.parent,type:'googlePlusSignIn',data:data,origin:originUrl});}
function googlePlusButtonRender(){gapi.signin.render('customButton',{'callback':'googlePlusSignInCallback','clientid':google_api_key,'cookiepolicy':'single_host_origin','scope':'profile email','accesstype':'offline'});}
function googlePlusLoadJS(){var script=document.createElement("script");script.type="text/javascript";script.src="//web.archive.org/web/20180902055137/https://apis.google.com/js/client:plusone.js?onload=googlePlusButtonRender";document.body.appendChild(script);}
pm.bind('googlePlusLoadJS',googlePlusLoadJS,originUrl);

}
/*
     FILE ARCHIVED ON 05:51:37 Sep 02, 2018 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 15:55:02 Mar 26, 2021.
...
playback timings (ms):
  ...
  load_resource: 736.634 (2)
  esindex: 0.026
  captures_list: 6482.53
  LoadShardBlock: 2720.476 (4)
  PetaboxLoader3.resolve: 607.262 (2)
  PetaboxLoader3.datanode: 324.59 (6)
  exclusion.robots.policy: 0.248
  CDXLines.iter: 280.131 (4)
*/