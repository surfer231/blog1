...
if ( isSecureConnection() ) @ini_set( 'session.cookie_secure', 1 );
...
function getImportInfo( $importFile, $cleanup = false )
{
  ...
  for ( $i = 0; $i < $zip->numFiles; $i++ ) {
    if ( $i >= 100 ) break; // to avoid hanging on non valid zip with a lot of files
...
$taskStats            = !empty( $_POST['taskStats'] ) ? trim( $_POST['taskStats'] ) : serialize( [] );
                    ...
                    <form action="" method="post" id="form_acms_settings" class="needs-validation" novalidate>
                      <?php if ( !inSafeMode() ) { ?>
                        ...
                        <div class="form-group">
                          ...
                          <div class="alert-danger rounded p-2 mt-1 small d-none" id="div_acms_settings_passwords_match"><?=L( 'Important! The administrative password and the safe mode password should not be the same. Otherwise, you will not be able to log in as an administrator. There is no need to set a safe mode password if you\'re not providing the access to someone else.' )?></div>
